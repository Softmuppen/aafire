...
pkgdatadir = $(datadir)/aalib
pkglibdir = $(libdir)/aalib
pkgincludedir = $(includedir)/aalib
...
AALIB_CFLAGS = 
AALIB_LIBS = -laa -lm   -L/usr/lib64 -lX11 -lgpm  -lslang
...
m4data_DATA = aalib.m4
ACLOCAL_M4 = $(top_srcdir)/aclocal.m4
mkinstalldirs = $(SHELL) $(top_srcdir)/mkinstalldirs
CONFIG_HEADER = ./src/config.h
CONFIG_CLEAN_FILES =  aalib-config
SCRIPTS =  $(bin_SCRIPTS)

DATA =  $(m4data_DATA)

DIST_COMMON =  README AUTHORS COPYING ChangeLog INSTALL Makefile.am \
Makefile.in NEWS aalib-config.in acconfig.h aclocal.m4 config.guess \
config.sub configure configure.in install-sh ltconfig ltmain.sh missing \
mkinstalldirs src/config.h.in src/stamp-h.in texinfo.tex
...
aalib-config: $(top_builddir)/config.status aalib-config.in
